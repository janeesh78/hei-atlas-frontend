...
| CC3 — Risk assessment | Draft | First risk assessment drafted 2026-09-04 (`RISK_ASSESSMENT.md`), NIST 800-30 methodology — not yet adopted, needs counsel review and an annual re-assessment cadence going forward. |
...
- [x] **Neon** — turned out to be self-serve, not an email request: org-level "HIPAA support" enabled directly in the Neon console 2026-09-04 (click-through BAA accepted via the toggle itself, no separate signature exchange; PDF reviewed, Databricks-letterheaded template naming Neon LLC as the party, consistent with the Databricks/Neon acquisition). Confirmed same day via the project dashboard: Scale plan ✓, and — the thing that kept this open initially — HIPAA compliance shows **Enabled at the project level** (`ep-empty-violet-atovl6j8-pooler`), not just the org. Both requirements met.
...
- [x] **Neon** — mark the relevant database(s) as HIPAA databases via their self-serve enablement process (Scale plan). Core Postgres, branching, backups, and PITR are covered; Neon Auth and the Data API are explicitly outside the HIPAA boundary regardless — don't route PHI through those even after signing. Confirmed done 2026-09-04 — project dashboard shows HIPAA compliance Enabled on the actual project, not just the org.
...
- [ ] Complete an initial risk assessment (methodology: NIST 800-30 or SANS). First draft: [`RISK_ASSESSMENT.md`](RISK_ASSESSMENT.md) (2026-09-04, NIST 800-30 methodology, 12-item risk register) — not adopted, needs counsel review; identifies entity classification (below) as its single highest-rated open risk.
- [ ] Draft and publish a Notice of Privacy Practices. First draft: [`NOTICE_OF_PRIVACY_PRACTICES.md`](NOTICE_OF_PRIVACY_PRACTICES.md) (2026-09-04) — not adopted, needs counsel review and the entity-classification call below resolved first (that draft explains why the classification changes who this document is even for).
